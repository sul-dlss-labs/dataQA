...
class Record:
    ...
    def get_record_id(self):
        ...
        try:
#            id_node = self.elem.find("teiHeader/fileDesc/sourceDesc/msDesc/msIdentifier/altIdentifier[@type='resource']/idno", namespaces=namespaces)
#            record_id = id_node.text
            record_id = 'id'
            return record_id
        except:
    ...
    def get_elements(self):
        ...
        if len(metadata):
            for desc in metadata.iterdescendants():
                if (desc.tag == TEI_NS + self.args.element) and desc.text:
                ...
                if metadata.xpath(self.args.xpath, namespaces=namespaces):
                    for value in metadata.xpath(self.args.xpath, namespaces=namespaces):
                        try:
                            ...
                            out.append(value.encode("utf-8").strip())
                        except:
                            pass
                if len(out) == 0:
                    out = None
    ...
    def get_stats(self):
        ...
        stats = {}
        metadata = self.elem
        tei = etree.ElementTree(metadata)
        if len(metadata):
            for desc in metadata.iterdescendants():
                if len(desc) == 0 and desc.text:
                    # ignore empties, does NOT have children elements
                    try:
                        stats.setdefault(re.sub('\[\d+\]','', tei.getelementpath(desc).replace(TEI_NS, 'tei:')), 0)
                        stats[re.sub('\[\d+\]','', tei.getelementpath(desc).replace(TEI_NS, 'tei:'))] += 1
                    except:
                        print("Error: " + desc)
        return stats
