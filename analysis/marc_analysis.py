...
class Record:
    ...
    def __init__(self, elem, args):
        self.elem = json.loads(elem)
        self.args = args

    def get_record_id(self):
        try:
            record_id = self.elem['fields'][0]['001']
            return record_id
        except:
    ...
    def get_elements(self):
        out = []
        try:
            record = objectpath.Tree(self.elem)['fields']
    ...
    def get_stats(self):
        stats = {}
        for n in self.elem['fields']:
            for field, value in n.iteritems():
                if isinstance(value, dict):
                    ind1 = value['ind1']
                    ind2 = value['ind2']
                    subfields = value['subfields']
                    out = field + '_' + ind1 + ind2 + '_'
                    for subfield in subfields:
                        for subfield1, value2 in subfield.iteritems():
                            out += '$' + subfield1
                    stats.setdefault(out ,0)
                    stats[out] += 1
                else:
                    if field != None:
...
def main():
    ...
    s = 0
    with open(args.datafile, 'rb') as data:
        reader = pymarc.MARCReader(data)
        for rec in reader:
            record = Record(rec.as_json(), args)
