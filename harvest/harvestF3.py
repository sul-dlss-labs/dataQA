...
def main():
    ...
    parser.add_argument("-l", "--link", dest="link", help="Fedora Base")
    parser.add_argument("-o", "--filename", dest="fname",
                        help="write repository to file", default="harvest.xml")
    parser.add_argument("pids", help="txt file with 1 pid per line")
    args = parser.parse_args()

    # Check OAI-PMH URL is valid
    if not args.link.startswith('https'):
        args.link = 'https://' + args.link

    # Start Harvest Process
    print("Writing records to %s from %s" % (args.fname, args.link))
